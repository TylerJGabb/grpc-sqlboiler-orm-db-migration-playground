...
func (r *videoR) GetUsers() UserSlice {
	if r == nil {
		return nil
	}
	return r.Users
...
var (
	videoAllColumns            = []string{"id", "name"}
	videoColumnsWithoutDefault = []string{"name"}
...
func (o *Video) Users(mods ...qm.QueryMod) userQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"video_tags\" on \"users\".\"id\" = \"video_tags\".\"user_id\""),
...
func (videoL) LoadUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeVideo interface{}, mods queries.Applicator) error {
	var slice []*Video
	var object *Video
	...
	query := NewQuery(
		qm.Select("\"users\".\"id\", \"users\".\"name\", \"a\".\"video_id\""),
		qm.From("\"users\""),
		qm.InnerJoin("\"video_tags\" as \"a\" on \"users\".\"id\" = \"a\".\"user_id\""),
		qm.WhereIn("\"a\".\"video_id\" in ?", argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Video) AddUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into \"video_tags\" (\"video_id\", \"user_id\") values ($1, $2)"
...
// Sets o.R.Videos's Users accordingly.
// Replaces o.R.Users with related.
// Sets related.R.Videos's Users accordingly.
func (o *Video) SetUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	...
	removeUsersFromVideosSlice(o, related)
	if o.R != nil {
		o.R.Users = nil
	}
...
func (o *Video) RemoveUsers(ctx context.Context, exec boil.ContextExecutor, related ...*User) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"video_tags\" where \"video_id\" = $1 and \"user_id\" in (%s)",
...
func removeUsersFromVideosSlice(o *Video, related []*User) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
...
func (o *Video) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(videoPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert videos, could not build conflict column list")
			}

			conflict = make([]string, len(videoPrimaryKeyColumns))
			copy(conflict, videoPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"videos\"", updateOnConflict, ret, update, conflict, insert, opts...)
