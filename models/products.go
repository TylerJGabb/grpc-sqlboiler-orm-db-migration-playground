...
func (r *productR) GetInvoices() InvoiceSlice {
	if r == nil {
		return nil
	}
	return r.Invoices
...
var (
	productAllColumns            = []string{"id", "sku"}
	productColumnsWithoutDefault = []string{}
	productColumnsWithDefault    = []string{"id", "sku"}
...
func (o *Product) Invoices(mods ...qm.QueryMod) invoiceQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"invoice_items\" on \"invoices\".\"id\" = \"invoice_items\".\"invoice_id\""),
		qm.Where("\"invoice_items\".\"product_id\"=?", o.ID),
	)
...
func (productL) LoadInvoices(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProduct interface{}, mods queries.Applicator) error {
	var slice []*Product
	var object *Product
	...
	for results.Next() {
		...
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice invoices")
		}
...
func (o *Product) AddInvoices(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Invoice) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into \"invoice_items\" (\"product_id\", \"invoice_id\") values ($1, $2)"
...
// Sets o.R.Products's Invoices accordingly.
// Replaces o.R.Invoices with related.
// Sets related.R.Products's Invoices accordingly.
func (o *Product) SetInvoices(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Invoice) error {
	query := "delete from \"invoice_items\" where \"product_id\" = $1"
	...
	removeInvoicesFromProductsSlice(o, related)
	if o.R != nil {
		o.R.Invoices = nil
	}
...
func (o *Product) RemoveInvoices(ctx context.Context, exec boil.ContextExecutor, related ...*Invoice) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"invoice_items\" where \"product_id\" = $1 and \"invoice_id\" in (%s)",
...
func removeInvoicesFromProductsSlice(o *Product, related []*Invoice) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
